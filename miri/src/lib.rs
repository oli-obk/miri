...
pub fn eval_main<'a, 'tcx: 'a>(
    ...
    let mut ecx = rustc_miri::interpret::EvalContext::new(tcx, limits, Default::default(), Default::default());
...
struct EvalContext<'b, 'a: 'b, 'tcx: 'a>(&'b mut rustc_miri::interpret::EvalContext<'a, 'tcx, Evaluator>);

impl<'b, 'a, 'tcx> std::ops::Deref for EvalContext<'b, 'a, 'tcx> {
    type Target = rustc_miri::interpret::EvalContext<'a, 'tcx, Evaluator>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}

impl<'b, 'a, 'tcx> std::ops::DerefMut for EvalContext<'b, 'a, 'tcx> {
    fn deref_mut (&mut self) -> &mut Self::Target {
        &mut self.0
    }
}

impl<'b, 'a, 'tcx> EvalContext<'b, 'a, 'tcx> {
    fn call_c_abi(
        &mut self,
        def_id: DefId,
        ...
        match &link_name[..] {
            ...
            "unsetenv" => {
                let mut success = None;
                {
                    ...
                    if !name_ptr.is_null()? {
                        let name = self.0.memory.read_c_str(name_ptr.to_ptr()?)?;
                        if !name.is_empty() && !name.contains(&b'=') {
                            success = Some(self.0.machine_data.env_vars.remove(name));
                        }
                    }
                }
            ...
            "pthread_key_create" => {
                ...
                let key = MemoryMut(&mut self.memory).create_tls_key(dtor) as u128;
            ...
            "pthread_key_delete" => {
                ...
                MemoryMut(&mut self.memory).delete_tls_key(key)?;
                // Return success (0)
                self.write_null(dest, dest_ty)?;
            }
            "pthread_getspecific" => {
                // The conversion into TlsKey here is a little fishy, but should work as long as usize >= libc::pthread_key_t
                let key = self.value_to_primval(args[0], usize)?.to_u64()? as TlsKey;
                let ptr = MemoryMut(&mut self.memory).load_tls(key)?;
            ...
            "pthread_setspecific" => {
                ...
                MemoryMut(&mut self.memory).store_tls(key, new_ptr)?;
    ...
    fn finish(&mut self) -> EvalResult<'tcx> {
        let mut dtor = MemoryMut(&mut self.memory).fetch_tls_dtor(None)?;
        ...
        while let Some((instance, ptr, key)) = dtor {
            ...
            dtor = match MemoryMut(&mut self.memory).fetch_tls_dtor(Some(key))? {
                dtor @ Some(_) => dtor,
                None => MemoryMut(&mut self.memory).fetch_tls_dtor(None)?,
            };
        }
        Ok(())
    }
...
/*
impl<'b, 'a, 'tcx> std::ops::Deref for Memory<'b, 'a, 'tcx> {
    type Target = rustc_miri::interpret::Memory<'a, 'tcx, Evaluator>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
...
impl<'b, 'a, 'tcx> std::ops::Deref for MemoryMut<'b, 'a, 'tcx> {
    type Target = rustc_miri::interpret::Memory<'a, 'tcx, Evaluator>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
...
impl<'b, 'a, 'tcx> std::ops::DerefMut for MemoryMut<'b, 'a, 'tcx> {
    fn deref_mut (&mut self) -> &mut Self::Target {
        &mut self.0
    }
}

impl<'b, 'a, 'tcx> MemoryMut<'b, 'a, 'tcx> {
