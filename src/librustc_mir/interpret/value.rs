...
impl<'a, 'tcx: 'a> Value {
    ...
    pub fn into_ptr<M: Machine<'tcx>>(&self, mem: &mut Memory<'a, 'tcx, M>) -> EvalResult<'tcx, Pointer> {
        use self::Value::*;
        match *self {
            ByRef(ptr, aligned) => {
    ...
    pub(super) fn into_slice<M: Machine<'tcx>>(&self, mem: &mut Memory<'a, 'tcx, M>) -> EvalResult<'tcx, (Pointer, u64)> {
        use self::Value::*;
        match *self {
            ByRef(ref_ptr, aligned) => {
